...
module Router #(
    ...
    // これは、トークン・コントローラーが、書き込んでいるバッファが一杯になったので停止するように、と言っていることに戻る。
    ...
    FromLocal #(
        ...
        .empty_east(empty_local_to_east),   // Output -- Lets merge in forward east know if the buffer going to it is empty フォワード・イーストのマージにバッファが空かどうかを知らせる。
        .empty_west(empty_local_to_west),   // Output -- Lets merge in forward west know if the buffer going to it is empty
        
        .buffer_full(local_buffers_full)
    ...
    ForwardEastWest #(
        ...
        .routing_empty(empty_out_east),          // Output -- Lets the forward east module of the core to the east know if the buffer going to it is empty 東側コアのforward east moduleに向かうバッファが空かどうかを知らせる
        .north_empty(empty_east_to_north),       // Output -- Lets the forward north module know if the buffer going to it is empty
        .south_empty(empty_east_to_south)        // Output -- Lets the forward south module know if the buffer going to it is empty
    );
    ...
    ForwardEastWest #(
        ...
        .routing_empty(empty_out_west),          // Output -- Lets the forward west module of the core to the west know if the buffer going to it is empty
        .north_empty(empty_west_to_north),       // Output -- Lets the forward north module know if the buffer going to it is empty
        .south_empty(empty_west_to_south)        // Output -- Lets the forward south module know if the buffer going to it is empty                       
    );
    ...
    ForwardNorthSouth #(
        ...
        .empty_routing(empty_in_south),                 // Input  -- Lets merge know if the buffer in the south core is empty 南コアのバッファが空であるならマージモジュールに知らせる
        ...
        .ren_out_east(ren_north_to_east),               // Output -- Read enable to the buffer in the forward east module 　forward_eastモジュール内バッファへの書き込み許可信号
        ...
        .routing_empty(empty_out_north),         // Output -- Lets the forward north module of the core to the north know if the buffer going to it is empty
        .local_empty(empty_north_to_local)       // Output -- Lets the local in module of this core know if the buffer going to it is empty
    );
    ...
    ForwardNorthSouth #(
        ...
        .routing_empty(empty_out_south),         // Output -- Lets the forward south module of the core to the south know if the buffer going to it is empty
        .local_empty(empty_south_to_local)       // Output -- Lets the local in module of this core know if the buffer going to it is empty
    );
    
    LocalIn #(
        .PACKET_WIDTH(PACKET_WIDTH-(DX_MSB-(DY_LSB-1)))//ここでのパラメータ指定で、dxとdyの情報は破棄され出力パケットを12ビットに変換
    ) local_in (
        .clk(clk),
        .rst(rst),
