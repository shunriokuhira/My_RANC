...
module PathDecoder3Way#(
    ...
    input [DATA_WIDTH-1:0] dx_dy,//from buffer
    ...
    assign dx = dx_dy[DX_MSB:DX_LSB];
    assign dy = dx_dy[DX_MSB:DX_LSB];
    ...
    assign empty_a = dx == 0 ? 1 : empty;//dx==0とはそれ以上水平方向にパケットを移動しないでいいということ

    //to north_buffer(北)
    assign dout_b = din[DX_LSB-1:0];
    //assign empty_b = dy >= 0 ? (dx == 0 ? empty : 1) : 1;
    assign empty_b = (dy >= 0 && dx == 0) ? empty : 1; 
    
    //to south_buffer(南)
    assign dout_c = din[DX_LSB-1:0];
    assign empty_c = dy < 0 ? (dx == 0 ? empty : 1) : 1;
